...
import { restaurants, menuCategories, menuItems, departments } from './src/lib/db/schema';
import { eq, and } from 'drizzle-orm';

/**
 * Расширенный seed скрипт с тестовыми данными из проекта resto
 * Добавляет множество реалистичных блюд с изображениями, калориями и описаниями
 */
...
async function seedExtended() {
  try {
    ...
    // Создаем ресторан test1 для Turso
    ...
    let zakuskiCat = await db.query.menuCategories.findFirst({
      where: and(eq(menuCategories.restaurantId, restaurant.id), eq(menuCategories.name, 'Закуски')),
    });
    if (!zakuskiCat) {
      [zakuskiCat] = await db.insert(menuCategories).values({
    ...
    let saladesCat = await db.query.menuCategories.findFirst({
      where: and(eq(menuCategories.restaurantId, restaurant.id), eq(menuCategories.name, 'Салаты')),
    });
    if (!saladesCat) {
      [saladesCat] = await db.insert(menuCategories).values({
        restaurantId: restaurant.id,
        name: 'Салаты',
    ...
    let soupsCat = await db.query.menuCategories.findFirst({
      where: and(eq(menuCategories.restaurantId, restaurant.id), eq(menuCategories.name, 'Супы')),
    });
    if (!soupsCat) {
      [soupsCat] = await db.insert(menuCategories).values({
        restaurantId: restaurant.id,
        name: 'Супы',
    ...
    let hotCat = await db.query.menuCategories.findFirst({
      where: and(eq(menuCategories.restaurantId, restaurant.id), eq(menuCategories.name, 'Горячее')),
    });
    if (!hotCat) {
      [hotCat] = await db.insert(menuCategories).values({
        restaurantId: restaurant.id,
        name: 'Горячее',
    ...
    let desertsCat = await db.query.menuCategories.findFirst({
      where: and(eq(menuCategories.restaurantId, restaurant.id), eq(menuCategories.name, 'Десерты')),
    });
    if (!desertsCat) {
      [desertsCat] = await db.insert(menuCategories).values({
        restaurantId: restaurant.id,
        name: 'Десерты',
    ...
    let drinksCat = await db.query.menuCategories.findFirst({
