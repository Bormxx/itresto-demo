...
export default async function KitchenPage({
  ...
  // Разрешенные роли для доступа к странице приготовления
  const allowedRoles = ['kitchen_staff', 'manager', 'supervisor', 'admin'];
  
  if (!session?.user || !allowedRoles.includes(session.user.role)) {
    const { restaurant } = await params;
    redirect(`/${restaurant}/auth/signin?callbackUrl=/${restaurant}/kitchen`);
  }
  ...
  // Только для staff с ролями приготовления проверяем назначение в смене
  if (session.user.role === 'kitchen_staff') {
    ...
    // Для manager/supervisor показываем всю кухню (не Бар)
    const kitchenDept = await db.query.departments.findFirst({
      where: and(
        eq(departments.restaurantId, session.user.restaurantId),
        eq(departments.name, 'Кухня'),
        eq(departments.isFoodPreparation, true)
      ),
    });
    staffDepartmentId = kitchenDept?.id || null;
    ...
    with: {
      ...
      orderItems: {
        where: or(
          ...
          eq(orderItems.kitchenStatus, 'ready'),
          eq(orderItems.kitchenStatus, 'delivered')
        ),
        with: {
          menuItem: {
            with: {
  ...
  const sortedOrders = departmentOrders.sort((a, b) => {
    const minPriorityA = Math.min(...a.orderItems.map(item => getStatusPriority(item.kitchenStatus)));
    const minPriorityB = Math.min(...b.orderItems.map(item => getStatusPriority(item.kitchenStatus)));
