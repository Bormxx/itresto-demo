...
export function BarDashboard({ initialOrders, notAssigned = false }: Props) {
  ...
  const pendingCount = orders.filter(order => 
    order.orderItems.some(item => item.barStatus === 'pending')
  ).length;
    ...
    <>
      ...
      <div className="mb-4 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex-1 max-w-md">
          <input
            type="text"
            placeholder="Поиск по заказу, столику, блюду..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full rounded-lg border border-[#d1d5db] px-4 py-2 text-sm focus:border-[#3b82f6] focus:outline-none"
          />
        </div>
      </div>
      ...
      <div>
        <section>
          <h2 className="mb-4 text-xl font-bold text-[#111827]">
            Все заказы ({filteredOrders.length})
          </h2>
